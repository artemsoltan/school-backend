...
public class AdminService implements AdminServiceImpl, SchoolServiceImpl {
    ...
    public AdminService(PersonRepository personRepository, SubjectRepository subjectRepository, JwtUtil jwtUtil, PasswordEncoder passwordEncoder, SchoolClassRepository schoolClassRepository, SchoolRepository schoolRepository, SaveUserService saveUserService) {
    ...
    public List<Person> getAllTeachers(String username) {
        Person person = personRepository.findByUsername(username).orElse(null);
        if (person != null) {
            return personRepository.findAllBySchool(person.getSchool()).orElse(null);
        }
        return null;
    }
